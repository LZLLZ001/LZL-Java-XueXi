...
快速入门(了解)
    ...
    2. 导入JQuery的js文件：导入min.js文件(一般都会自己建一个JS目录来放JS文件)
            <script src="js/jquery-3.3.1.min.js(文件目录)"></script>
    3. 使用
        var div1 = $("#div1");


JQuery对象和JS对象区别与转换
	1. JQuery对象在操作时，更加方便，因为有很多已经写好了的方法可以直接调用(面向对象)。
...
选择器：筛选具有相似特征的元素(标签)，选择器选出来的就是jq对象
	1. 基本操作学习：
		(1) 事件绑定
			//1.获取b1按钮
            $("#b1").click(function(){
    ...
    2. 分类
		...
		(2) 层级选择器
			...
			2. 子选择器
				* 语法： $("A > B") 选择A元素内部的所有B子元素(只有一层,B下的孙子B不包含)
		(3) 属性选择器
			1. 属性名称选择器
				* 语法： $("A[属性名]") 包含指定属性的选择器
			2. 属性选择器
...
DOM操作
	...
	2. 属性操作
		(1) 通用属性操作
			...
			3. prop():获取/设置元素的属性
			4. removeProp():删除属性
                操作的是元素的固有属性使用prop，操作的是元素自定义的属性使用attr
		(2) 对class属性操作
