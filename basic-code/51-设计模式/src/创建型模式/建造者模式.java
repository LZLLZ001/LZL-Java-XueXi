...
建造者模式：
    ...
    建造者模式是一步一步创建一个复杂的对象，它允许用户只通过指定复杂对象
...
建造者模式的四个角色
    ...
    2) Builder（抽象建造者）：创建一个Product对象的各个部件指定的 接口/抽象类。只指定建造流程，不管建造细节。
    3) ConcreteBuilder（具体建造者）：实现接口，构建和装配各个部件。实现各个建造细节。
    4) Director（指挥者）：构建一个使用Builder的对象。它主要是用于创建一个复杂的对象。
